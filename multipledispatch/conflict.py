...
def supercedes(a, b):
    ...
    if len(a) < len(b):
        ...
        return all(map(safe_subtype, a, b))
    else:
        # len(a) > len(b)
        p1 = 0
        p2 = 0
        while p1 < len(a) and p2 < len(b):
            ...
            if not (isvariadic(cur_a) or isvariadic(cur_b)):
                if not safe_subtype(cur_a, cur_b):
                    return False
                p1 += 1
                p2 += 1
            elif isvariadic(cur_a):
                assert p1 == len(a) - 1
                return p2 == len(b) - 1 and safe_subtype(cur_a, cur_b)
...
def consistent(a, b):
    ...
    if len(a) == len(b):
        return all(safe_subtype(aa, bb) or safe_subtype(bb, aa)
        ...
        while p1 < len(a) and p2 < len(b):
            cur_a = a[p1]
            cur_b = b[p2]
            if not safe_subtype(cur_b, cur_a) and not safe_subtype(cur_a, cur_b):
                return False
            if not (isvariadic(cur_a) or isvariadic(cur_b)):
                p1 += 1
                p2 += 1
